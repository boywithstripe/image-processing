...
int main()
{
	//Mat image = imread("E:\\学习资料\\研一下\\视频图像处理_茅\\homework_NeedHandIn\\火焰检测\\image7.jpg");
	Mat image = cv::imread("image1.jpg");
	if (image.empty()) {
		cout << "图片读取失败！" << endl;
	}
	imshow("Original Image", image);
	//imshow("firedetection", FireDetection(image));    //第一种方法，效果不好
	imshow("firdetection", FireDetectionHSV(image));
	imshow("firdetection_RGB", image);
	waitKey(0);
	return 0;
}


//这里没有用到直方图，可以当作没有这个东西
void ShowHistogram(Mat& Orimage) 
{
	...
	*     
	*************/
	/* 绘制出问题了，但是这不重要，直接利用上述输出信息查看即可,有兴趣可以去修改修改
	for (int i = 0; i < 256; i++)
	{
